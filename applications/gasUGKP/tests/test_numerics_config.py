...
CREATE_FIELDS = (
    ...
    ("std::int32_t", "csrSplitPreDirectoryEnabled"),
    ("std::int32_t", "dragModel"),
...
PROTOCOL_CREATE_FIELDS = CREATE_FIELDS
...
class UGKPNumericsConfiguration(unittest.TestCase):
    ...
    def test_create_payload_field_order_and_size(self) -> None:
        ...
        self.assertEqual(actual, list(PROTOCOL_CREATE_FIELDS))
        self.assertIn(
            'static_assert(sizeof(CreateArgs) == 392, "unexpected create payload ABI")',
            self.protocol,
        )

    def test_c_abi_create_signatures_match_protocol_exactly(self) -> None:
        expected = list(CREATE_FIELDS) + [("void**", "handle")]
        layers = {
            "public API": self.api,
    ...
    def test_client_payload_and_server_forwarding_are_exact(self) -> None:
        ...
        self.assertEqual(
            ...
            [f"a.{name}" for _, name in CREATE_FIELDS] + ["&state.backend"],
        )
    ...
    def test_pure_gas_validation_launches_ugkp(self) -> None:
        ...
        for path in launchers + controls:
            ...
            if "diluteUgkwpFoam_" in source:
                with self.subTest(path=path.relative_to(ROOT)):
                    self.assertIn("UGKP", source)
                    self.assertNotIn("legacySolverName", source)

    def test_current_pure_gas_gate_contains_first_cpu_write(self) -> None:
        gas_case = ROOT / "examples/pressuretest/GPU_gas/hllc"
        if not gas_case.is_dir():
            self.skipTest("clean UGKP source package has no legacy examples")
        control = (gas_case / "system/controlDict").read_text()
        self.assertRegex(control, r"\bendTime\s+0\.1\s*;")
        self.assertRegex(control, r"\bwriteInterval\s+0\.01\s*;")
        self.assertTrue((gas_case / "0.010000023").is_dir())
        cpu_write = (
