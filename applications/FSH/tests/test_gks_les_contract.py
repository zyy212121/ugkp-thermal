...
class SourceContractTests(unittest.TestCase):
    ...
    def test_create_and_courant_ipc_abi_contract(self) -> None:
        ...
        self.assertIn('sizeof(CreateArgs) == 352', protocol)
        self.assertRegex(
            protocol,
    ...
    def test_csr_hierarchical_experiment_configuration_contract(self) -> None:
        ...
        self.assertIn("dynamicHeavyThreshold", cuda)
        self.assertIn("configureLaunchOccupancy", cuda)
    ...
    def test_csr_heavy_and_warp_aggregation_source_contract(self) -> None:
        ...
        self.assertNotRegex(
            cuda,
            r"finalizeCsrHeavy(?:Pool|Moment)CellsKernel\s*<<<\s*s->nCells",
        )
        self.assertNotRegex(
            cuda,
            r"cudaMemcpy(?:Async)?\s*\([^;]*csrHeavyTaskCount",
        )
        self.assertIn("ToolB3 maximum occupancy", cuda)
    ...
    def test_diffusion_validation_uses_contact_preserving_flux(self) -> None:
        if not (ROOT / "examples/gks_flux_validation").is_dir():
            self.skipTest("clean UGKP source package has no legacy examples")
        for case in (
            "planarCouette",
            "fourierSlab",
