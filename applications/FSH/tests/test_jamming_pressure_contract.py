...
class MobilePackingProjectionSourceContractTests(unittest.TestCase):
    ...
    def test_final_particle_velocity_writeback_excludes_stuck_particles(self):
        eligibility = braced_block_after(
            self.source,
            r"\b__device__\s+bool\s+mobilePackingParticleEligible\s*\(",
        )
        self.assertIn("s.pStatus[i] == 1", eligibility)
        self.assertIn("s.pStuck[i] == 0", eligibility)

        particle = braced_block_after(
            self.packing_cooperative,
            r"\b__device__\s+void\s+"
            r"applyMobilePackingCorrectionToParticleDevice\s*\(",
        )
        self.assertIn("!mobilePackingParticleEligible(s, i)", particle)
        guard = particle.index("!mobilePackingParticleEligible(s, i)")
        for field in ("pux", "puy", "puz", "puxOld", "puyOld", "puzOld"):
            write = f"s.{field}[i] ="
            with self.subTest(write=write):
                self.assertIn(write, particle)
                self.assertLess(guard, particle.index(write))
    ...
    def test_local_projection_has_one_correction_halo(self):
        ...
        for field in (
            ...
            "mobilePackingCorrectionCellList",
            "mobilePackingCorrectionCellCount",
