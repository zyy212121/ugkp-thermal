...
FSH_STATE_FIELDS = ("pStuck", "pStuckFaceId", "pDepositionArea")
INVARIANT_NAMES = (
    "mass", "mom_x", "mom_y", "mom_z", "energy", "mass_d", "mass_T",
    "represented_deposition_area",
)
        ...
        values = np.column_stack(
            (
                ...
                np.where(
                    chunk["pStuck"] != 0,
                    m
                    * chunk["pDepositionArea"].astype(np.longdouble)
                    / np.maximum(
                        chunk["pd"].astype(np.longdouble) ** 3,
...
def _refined_chunks(
    ...
    for chunk in iter_restart_chunks(source, legacy_parcel_mass):
        ...
        for name in FSH_STATE_FIELDS:
            if not np.array_equal(refined[name], chunk[name][base]):
                raise RuntimeError(f"FSH state {name} changed during refinement")
