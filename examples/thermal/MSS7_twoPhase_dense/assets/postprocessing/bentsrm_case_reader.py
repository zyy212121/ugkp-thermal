...
CASE = THERMAL / "bentSRM_coldWall"
OUTPUT = THERMAL / "results/bentSRM_coldWall"
...
def particle_chunks(path):
    ...
    for arrays in iter_fsh_chunks(path):
        count = arrays["cell"].size
        for name in ("contact_duration", "contact_maximum_area", "contact_peak_fraction", "cold_frozen_area"):
            arrays.setdefault(name, np.zeros(count))
        arrays.setdefault("cold_node_specific_enthalpy", np.zeros((count, 8)))
        yield arrays
...
def write_temperature_results():
    ...
    destination = OUTPUT_DATA / f"{PREFIX}_temperature_comparison.csv"
    ...
    configure_axes(axis)
    fig.tight_layout()
    fig.savefig(OUTPUT_FIGURES / f"{PREFIX}_temperature_comparison.png", dpi=600, facecolor="white")
    plt.close(fig)
...
def write_heat_flux_results():
    ...
    destination = OUTPUT_DATA / f"{PREFIX}_wall_heat_flux.csv"
    with destination.open("w", newline="", encoding="utf-8") as stream:
        writer = csv.writer(stream)
    ...
    configure_axes(axis)
    fig.tight_layout()
    fig.savefig(OUTPUT_FIGURES / f"{PREFIX}_wall_heat_flux.png", dpi=600, facecolor="white")
    plt.close(fig)
...
def write_sommerfeld_results():
    ...
    destination = OUTPUT_DATA / f"{PREFIX}_coupled_wall_incoming_sommerfeld.csv"
    with destination.open("w", newline="", encoding="utf-8") as stream:
        writer = csv.writer(stream)
    ...
    configure_axes(axis)
    fig.tight_layout()
    fig.savefig(OUTPUT_FIGURES / f"{PREFIX}_coupled_wall_incoming_sommerfeld.png",
...
def write_radiating_area_results():
    ...
    destination = OUTPUT_DATA / f"{PREFIX}_coupled_wall_radiating_area_fraction.csv"
    with destination.open("w", newline="", encoding="utf-8") as stream:
        writer = csv.writer(stream)
    ...
    configure_axes(axis)
    fig.tight_layout()
    fig.savefig(OUTPUT_FIGURES / f"{PREFIX}_coupled_wall_radiating_area_fraction.png",
                dpi=600, facecolor="white")
    plt.close(fig)
