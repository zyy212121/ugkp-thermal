...
COLD = RESULTS.parent / "bentSRM_coldWall"
...
def temperature_results():
    ...
    measured = measured[:, :2]
    with (RESULTS/f"{PREFIX}_temperature_comparison.csv").open("w", newline="", encoding="utf-8") as stream:
...
def heat_flux_results():
    ...
    with (RESULTS/f"{PREFIX}_wall_heat_flux.csv").open("w", newline="", encoding="utf-8") as stream:
        writer = csv.writer(stream)
...
def sommerfeld_results():
    ...
    with (RESULTS/f"{PREFIX}_coupled_wall_incoming_sommerfeld.csv").open("w", newline="", encoding="utf-8") as stream:
        writer = csv.writer(stream)
...
def radiating_area_results():
    ...
    with (RESULTS/f"{PREFIX}_coupled_wall_radiating_area_fraction.csv").open("w", newline="", encoding="utf-8") as stream:
        writer = csv.writer(stream)
...
def coupled_wall_particle_temperature_results():
    ...
    with (RESULTS/f"{PREFIX}_coupled_wall_particle_temperature.csv").open("w", newline="", encoding="utf-8") as stream:
        writer = csv.writer(stream)
    ...
    fig.tight_layout(pad=0.7)
    fig.savefig(RESULTS/f"{PREFIX}_coupled_wall_particle_temperature.png", dpi=600, facecolor="white")
    plt.close(fig)
